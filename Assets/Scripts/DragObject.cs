...
namespace PS01
{
    ...
    public class DragObject : MonoBehaviour
    {
        ...
        Vector3 initMousePosition = Vector3.zero;
        ...
        { 
            ...
            initMousePosition = lMousePosition;
            ...
            Debug.Log("OnMouseDown() myObjectStartPosition = " + initMousePosition);
        }
        ...
        private void OnMouseDrag()
        {
            ...
            if(lMousePosition == initMousePosition)
            {
                return;
            }
            //Vector3 lMouseCurrentWorldPosition = myMainCamera.ScreenToWorldPoint(lMousePosition);
            Vector3 diff = lMousePosition - initMousePosition;
            initMousePosition = lMousePosition;
            float dx = diff.x;
            float dy = diff.y;
            float dr = Mathf.Sqrt((dx * dx) + (dy * dy));
            Vector3 n = -new Vector3(-dy / dr, dx / dr, 0);
            ddd.transform.rotation *= Quaternion.Euler(n * 5);
            // transform.position = myObjectStartPosition + (lMouseCurrentWorldPosition - myMouseStartWorldPosition);
            // Debug.Log("OnMouseDown() lMousePosition = " + lMousePosition);
            Debug.Log("diffffff: " + n);
             //Debug.Log("OnMouseDrag() lMouseCurrentWorldPosition = " + lMousePosition);
